// JSON Schema to enforce structured CRM format output from OpenRouter according to PDF specs
...
/**
 * Standardizes a batch of raw records using OpenRouter API
 ...
 */
export async function processBatchWithOpenRouter(records, headers, apiKey, mappings = {}) {
    if (!apiKey) {
        throw new Error('OpenRouter API key is required. Please set it in your environment or provide it in the settings panel.');
    }
...
/**
 * Detects mapping from CSV columns to CRM fields using OpenRouter API
 * @param headers - Headers of the CSV
 * @param sampleRows - A few sample rows (2-3) of the CSV
 * @param apiKey - OpenRouter API Key
 * @returns Mapping dictionary: crmField -> csvHeader
 */
export async function detectSchemaWithOpenRouter(headers, sampleRows, apiKey) {
    if (!apiKey) {
        throw new Error('OpenRouter API key is required to detect schema.');
    }
    const model = process.env.OPENROUTER_MODEL || 'google/gemini-2.5-flash';
...
Return the schema mapping JSON representation.`;
    try {
            ...
            body: JSON.stringify({
                ...
                response_format: {
                    ...
                    json_schema: {
                        ...
                        strict: true,
                        schema: schemaDetectionResponseSchema
                    }
                },
                temperature: 0.1
            })
        ...
        const responseText = data.choices?.[0]?.message?.content;
        if (!responseText) {
            throw new Error('OpenRouter API returned an empty response during schema detection.');
        }
        const parsedResponse = JSON.parse(responseText);
        return (parsedResponse.mappings || {});
    }
    catch (error) {
        console.error('Error calling OpenRouter API for schema detection:', error);
